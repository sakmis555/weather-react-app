...
const Tomorrow = ({ city, setCity, checked, setChecked }) => {
    ...
    () => async () => {
      try {
        ...
        const response = await fetch(
          `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}/tomorrow?key=${process.env.REACT_APP_KEY}&include=days&iconSet=icons2`
        );
        if (!response.ok) {
          throw new Error(
